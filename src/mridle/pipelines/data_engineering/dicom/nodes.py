...
def aggregate_terminplanner(terminplanner_df: pd.DataFrame) -> pd.DataFrame:
    ...
    tp_df['gültig von'] = tp_df['gültig von'].fillna(pd.datetime('01.01.2014'))
    tp_df['gültig bis'] = tp_df['gültig bis'].fillna(pd.datetime('31.12.{}'.format(int(datetime.datetime.now().year
                                                                                       + 3))))
