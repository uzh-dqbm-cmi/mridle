...
def calc_idle_time_gaps(dicom_times_df: pd.DataFrame, tp_agg_df: pd.DataFrame, time_buffer_mins=0) -> pd.DataFrame:
    ...
    idle_df['day_start_tp'] = idle_df.apply(lambda x: datetime.datetime.combine(x['date'],
                                                                                pd.to_datetime(x['day_start_tp'])),
                                            axis=1)
    idle_df['day_end_tp'] = idle_df.apply(lambda x: datetime.datetime.combine(x['date'],
                                                                              pd.to_datetime(x['day_end_tp'])),
                                          axis=1)
